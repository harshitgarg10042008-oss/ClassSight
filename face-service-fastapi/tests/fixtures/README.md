...
The test does not expect near-1.0 confidence. It classifies a candidate as a `PRESENT` candidate only when the endpoint confidence is at least `0.60`; lower-confidence candidates remain review/no-match outcomes. The test fails if an enrolled person is not correctly matched as PRESENT or if an unenrolled face receives a PRESENT candidate.
